...
const sendNotification = async (io, notificationData) => {
    const { userId, title, message, type, link, userEmail, payload, btnText, attachments } = notificationData;
        ...
        if (userEmail) {
            ...
            const htmlContent = `
                <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 580px; margin: 0 auto; padding: 40px; border: 1px solid #e5ead7; border-radius: 30px; text-align: center; background-color: #fafbf5;">
                    <a href="${process.env.FRONTEND_URL}"><img src="${logoUrl}" alt="Royal Hotel" style="width: 100px; margin-bottom: 30px; border: 0;"></a>
                    ...
                    ${link ? `
                        <div style="margin-top: 30px;">
                            <a href="${link.startsWith('http') ? link : process.env.FRONTEND_URL + link}" 
                               style="display: inline-block; padding: 16px 36px; background-color: #2c332b; color: #ffffff !important; text-decoration: none; border-radius: 40px; font-weight: 800; font-size: 14px; letter-spacing: 1.5px; box-shadow: 0 10px 20px rgba(0,0,0,0.12);">
